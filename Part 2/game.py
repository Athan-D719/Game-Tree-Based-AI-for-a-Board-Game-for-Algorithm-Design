...
class Board:
    def __init__(self,width,height, p1_sprites, p2_sprites):
        ...
        # a Stack to store all previously made moves
        self.past_moves = Stack()
    ...
    def add_piece(self, row, col, player):
        if self.valid_move(row, col, player):
            self.past_moves.push(self.get_board())
    ...
    def undo(self):
        '''
        Undoes the current player's last move.

            Returns:
                undo_performed (bool): a boolean indicating if the current player's
                                        last move was successfully undone
        '''
        undo_performed = False
        if (len(self.past_moves) - 1 > 0):
            # two pops are required to go back to correct move
            self.past_moves.pop()
            self.set(self.past_moves.pop())
            undo_performed = True
        
        return undo_performed
...
player1_dropdown = Dropdown(900, 50, 200, 50, ['Human', bots[0].get_name()])
player2_dropdown = Dropdown(900, 110, 200, 50, ['Human', bots[1].get_name()])
player1_diff_dropdown = Dropdown(1100, 50, 100, 50, ['Easy', 'Medium', 'Hard'])
player2_diff_dropdown = Dropdown(1100, 110, 100, 50, ['Easy', 'Medium', 'Hard'])
grid_col = -1
grid_row = -1
choice = [None, None]
bot_difficulty = [2, 2]
while running:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            player1_diff_dropdown.handle_event(event)
            player2_diff_dropdown.handle_event(event)
            choice[0] = player1_dropdown.get_choice()
            choice[1] = player2_dropdown.get_choice()
            bot_difficulty[0] = player1_diff_dropdown.get_choice()
            bot_difficulty[1] = player2_diff_dropdown.get_choice()
            if event.type == pygame.MOUSEBUTTONDOWN:
                x,y = event.pos
                row = y - Y_OFFSET
                col = x - X_OFFSET    
                ...
                # checks if the current player wants to undo their last move
                if (event.key == pygame.K_z and not has_winner):
                    board.undo()
    ...
    if not has_winner:
        if overflowing:
            ...
            if choice[current_player] == 1:
                bots[current_player].set_difficulty(bot_difficulty[current_player])
    ...
    # Draw the bot difficulty selectors
    if choice[0] == 1:
        player1_diff_dropdown.draw(window)

    if choice[1] == 1:
        player2_diff_dropdown.draw(window)

    # Draw the undo instructions
    undo_font = pygame.font.Font(None, 36)
    undo_label = undo_font.render("Press Z to undo a move.", 1, BLACK)
    window.blit(undo_label, (850, 250))
